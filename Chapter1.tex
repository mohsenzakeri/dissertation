...
while maintaining the accuracy of Smith-Waterman. In the following sections, 
some of the common approaches will be discussed briefly.
...
Finding the seeds requires pre-processing the reference sequences 
...
Other aligners, such as deBGA~\citep{debga} and Minimap2~\citep{minimap2}, are based 
on hash-based indices. \Kmers of a string are defined as all the substrings 
...
Suffix Array to index the reference sequences~\citep{Dobin2013Star}. STAR uses some hash tables 
...
substrings of A or any other character in the BWT of a sequence compared to the original sequence. This property of the BWT makes it more compressible compared to the original sequence which results in smaller index size. 
Bowtie, Bowtie2, and BWA are popular aligners that index the reference sequences with a FM-Index.
...
The substrings of length k from the reference sequences are called \kmers. 
...
retrieve all the positions each \kmer occurs in the reference which will 
...
equal to 3. One main property in the de Bruijn graph is that every \kmer 
...
the graph, and if the edges are colored, not all the \kmers in a contig might 
have the same color.
...
table mapping each \kmer to the contig it is contained in and the position of the \kmer 
...
\begin{equation}
  ...
  \sum_{j=1}^{M} \Pr\left(t_j \mid \bm{\theta}\right) 
...
which walks through the space that the EM explores for finding the \mles of the transcripts expressions.
